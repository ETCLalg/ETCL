...
def train(args, model, device, x,y, optimizer, criterion, task_id_nominal, consolidated_masks, feature_matrix, per_task_masks, task_relation, epoch):
    ...
    for i in range(0,len(r),args.batch_size_train):
        ...
        for i in sims:
            ...
            output = model(data, i, mask=mask, mode="train", epoch=epoch, end_epoch=args.n_epochs//20)
        ...
        if consolidated_masks is not None and consolidated_masks != {}: # Only do this for tasks 1 and beyond
            ...
            if epoch < args.n_epochs//20:
                ...
                for key in feature_matrix.keys():
                    if 'last.' in key and str(task_id_nominal) not in key:
                        getattr(getattr(model,  'last'), key.split('.')[1]).weight.grad *= 0
...
def main(args):
    ...
    from dataloader import femnist_res
    data,taskcla,inputsize=femnist_res.get(seed=args.seed, pc_valid=args.pc_valid, idrandom=2, n_tasks=args.n_tasks)
    ...
    for k, ncla in taskcla:
        ...
        data_buffer.append(data[k]['train']['x'][0:args.sample_size])

        # print("get init dristribution for task {}...".format(k))
        mat = get_representation_matrix(model, device, data_buffer[task_id], task_id, None, size=(-1, 3, 32, 32))[task_id]
        ...
        for epoch in range(1, args.n_epochs+1):
            # Train
            clock0 = time.time()
            train(args, model, device, xtrain, ytrain, [backbone_optimizer, fc_optimizer], criterion, task_id, consolidated_masks, feature_mat, per_task_masks, (sim_tasks, dsim_tasks), epoch)
            clock1 = time.time()
        ...
        mat_list = get_representation_matrix(model, device, data_buffer[task_id], task_id, model.get_masks(task_id), size=(-1, 3, 32, 32))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n_tasks', type=int, default=35, metavar='S',
    ...
    # CSNB parameters
    parser.add_argument('--sparsity', type=float, default=0.4, metavar='SPARSITY',
    ...
    #ETIL parameters 
    parser.add_argument("--sim_threshold", default=0.28, type=float, help="Threshold for ETIL to determine similar tasks.")
    parser.add_argument("--k_threshold", default=0.95, type=float, help="Threshold for choose Orthogonal projection bases of SVD.")
