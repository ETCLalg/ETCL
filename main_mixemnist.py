...
from utils import *
from networks.mlp import SubnetMLPNet as MLP
...
def contrastive_calculate(model, task_id, task_relation):
    ...
    for s in sims:
        ...
        if current_fc_weight.shape[0] != sim_fc_weigth.shape[0]:
            continue
...
def train(args, model, device, x,y, optimizer, criterion, task_id_nominal, consolidated_masks, feature_matrix, per_task_masks, task_relation):
    ...
    for i in range(0,len(r),args.batch_size_train):
        ...
        for i in sims:
            if model.last[task_id_nominal].weight.shape[0] != model.last[i].weight.shape[0]:
                continue
            mask = per_task_masks[i]
            output = model(data, i, mask=mask, mode="train")
            if target.max() + 1 > output.shape[1]:
                continue
...
def main(args):
    ...
    from dataloader import mixemnist
    data,taskcla,inputsize=mixemnist.get(seed=args.seed, n_tasks=args.n_tasks, pc_valid=args.pc_valid, idrandom=args.idrandom)
    ...
    # Model Instantiation
    model = MLP(taskcla, args.sparsity, args.n_hidden, input_size=(28*28)) # base filters: 20
    ...
    init_mask = get_random_mask(model)
    feature_mat = OrderedDict() 
    data_buffer = []
    feature_list_ori = []
    ...
    for k, ncla in taskcla:
        ...
        mat = get_representation_matrix(model, device, data_buffer[task_id], task_id, init_mask, size=(-1, 28*28))[task_id]
        ...
        if task_id > 0:
            ...
            for i in range(task_id):
                mat_old_task = get_representation_matrix(model, device, data_buffer[i], i, per_task_masks[i], size=(-1, 28*28))[i]
                mat_new_task = get_representation_matrix(model, device, data_buffer[task_id], i, per_task_masks[i], size=(-1, 28*28))[task_id]
        ...
        mat_list = get_representation_matrix(model, device, data_buffer[task_id], task_id, model.get_masks(task_id), size=(-1, 28*28))
...
if __name__ == "__main__":
    ...
    # Model parameters
    parser.add_argument('--model', type=str, default="mlp", metavar='MODEL',
    ...
    #ETIL parameters 
    parser.add_argument("--sim_threshold", default=0.58, type=float, help="Threshold for ETIL to determine similar tasks.")
    parser.add_argument("--k_threshold", default=0.98, type=float, help="Threshold for choose Orthogonal projection bases of SVD.")
    parser.add_argument("--sample_size", default=500, type=int, help="Sample size for ETIL.")
