...
def get(seed=0, n_tasks=20, idrandom=3,pc_valid=0):
    size=[1,28,28]
    ...
    if n_tasks == 20:
        data_emnist, taskcla_emnist, size_emnist = read_emnist(seed=seed, pc_valid=pc_valid, n_tasks=10, class_per_task=5)
        data_femnist, taskcla_femnist, size_femnist = read_femnist(seed=seed, pc_valid=pc_valid, n_tasks=10)
    elif n_tasks == 40:
        data_emnist, taskcla_emnist, size_emnist = read_emnist(seed=seed, pc_valid=pc_valid, n_tasks=20, class_per_task=2)
        data_femnist, taskcla_femnist, size_femnist = read_femnist(seed=seed, pc_valid=pc_valid, n_tasks=20)

    all_emnist = [data_emnist[x]['name'] for x in range(n_tasks//2)]
    all_femnist = [data_femnist[x]['name'] for x in range(n_tasks//2)]

    f_name = 'mixemnist_random_'+str(n_tasks)
...
def read_emnist(seed=0,fixed_order=False,pc_valid=0.1, n_tasks=20, class_per_task=5):
    ...
    std=(0.3081,)
    dat={}
    dat['train']=datasets.EMNIST('./data/',train=True,download=True,split='balanced',transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
    dat['test']=datasets.EMNIST('./data/',train=False,download=True,split='balanced',transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
...
def read_femnist(seed=0,fixed_order=False,pc_valid=0.10,remain=0, n_tasks=20,):
    ...
    if n_tasks==10:
        train_dataset = FEMMNISTTrain(root_dir='./data/femnist/'+data_type+'/iid/train10/',transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
        dat['train'] = train_dataset

        test_dataset = FEMMNISTTest(root_dir='./data/femnist/'+data_type+'/iid/test10/',transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
        dat['test'] = test_dataset


    else:
        train_dataset = FEMMNISTTrain(root_dir='./data/femnist/'+data_type+'/iid/train20/',transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
        dat['train'] = train_dataset

        test_dataset = FEMMNISTTest(root_dir='./data/femnist/'+data_type+'/iid/test20/',transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
        dat['test'] = test_dataset
