...
def train(args, model, device, x,y, optimizer, criterion, task_id_nominal, consolidated_masks, feature_matrix, per_task_masks, task_relation):
    ...
    for i in range(0,len(r),args.batch_size_train):
        ...
        for i in sims:
            ...
            output = model(data, i, mask=mask, mode="train", size=(3, 32, 32))
            loss += criterion(output, target)

        loss += contrastive_calculate(model, task_id_nominal, task_relation)

        output = model(data, task_id_nominal, None, mode="train", size=(3, 32, 32))
        loss += criterion(output, target)
        loss.backward()
...
def test(args, model, device, x, y, criterion, task_id_nominal, curr_task_masks=None, mode="test", epoch=1):
    ...
    with torch.no_grad():
        ...
        for i in range(0,len(r),args.batch_size_test):
            ...
            data = x[b]
            data, target = data.to(device), y[b].to(device)
            output = model(data, task_id_nominal, mask=curr_task_masks, mode=mode, epoch=epoch, size=(3, 32, 32))
            loss = criterion(output, target)
...
def main(args):
    ...
    data,taskcla,inputsize=data_loader.get(seed=args.seed, pc_valid=args.pc_valid)
...
if __name__ == "__main__":
    ...
    #ETIL parameters 
    parser.add_argument("--sim_threshold", default=0.85, type=float, help="Threshold for ETIL to determine similar tasks.")
    parser.add_argument("--k_threshold", default=0.999, type=float, help="Threshold for choose Orthogonal projection bases of SVD.")
    parser.add_argument("--sample_size", default=150, type=int, help="Sample size for ETIL.")
