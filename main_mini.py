...
def train(args, model, device, x,y, optimizer, criterion, task_id_nominal, consolidated_masks, feature_matrix, per_task_masks, task_relation):
    ...
    for i in range(0,len(r),args.batch_size_train):
        ...
        for i in sims:
            ...
            output = model(data, i, mask=mask, mode="train", size=(3, 84, 84))
            loss += criterion(output, target)


        loss += contrastive_calculate(model, task_id_nominal, task_relation)

        output = model(data, task_id_nominal, None, mode="train", size=(3, 84, 84))
        loss += criterion(output, target)
        loss.backward()
...
def test(args, model, device, x, y, criterion, task_id_nominal, curr_task_masks=None, mode="test", epoch=1):
    ...
    with torch.no_grad():
        ...
        for i in range(0,len(r),args.batch_size_test):
            ...
            data = x[b]
            data, target = data.to(device), y[b].to(device)
            output = model(data, task_id_nominal, mask=curr_task_masks, mode=mode, epoch=epoch, size=(3, 84, 84))
            loss = criterion(output, target)
...
def main(args):
    ...
    for k, ncla in taskcla:
        ...
        data_buffer.append(data[task_id]['train']['x'][:args.sample_size])
        
        # print("get init dristribution for task {}...".format(k))
        mat = get_representation_matrix(model, device, data_buffer[task_id], task_id, init_mask, size=(-1,  3, 84, 84))[task_id]
        ...
        if task_id > 0:
            ...
            for i in range(task_id):
                mat_old_task = get_representation_matrix(model, device, data_buffer[i], i, per_task_masks[i], size=(-1,  3, 84, 84))[i]
                mat_new_task = get_representation_matrix(model, device, data_buffer[task_id], i, per_task_masks[i], size=(-1,  3, 84, 84))[task_id]
        ...
        mat_list = get_representation_matrix(model, device, data_buffer[task_id], task_id, model.get_masks(task_id), size=(-1,  3, 84, 84))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_new", default=True, help="Use new distribution or not", action="store_true")

    #ETIL parameters 
    parser.add_argument("--sim_threshold", default=0.95, type=float, help="Threshold for ETIL to determine similar tasks.")
    parser.add_argument("--k_threshold", default=0.99, type=float, help="Threshold for choose Orthogonal projection bases of SVD.")
    parser.add_argument("--sample_size", default=600, type=int, help="Sample size for ETIL.")
