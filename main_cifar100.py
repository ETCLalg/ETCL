...
def update_task_discrimination(task_id, feature_list_ori, feature_list_new, sim_tasks, dsim_tasks, threshold=0.7, sim_num=1):
    ...
    if len(sim_tasks[task_id]) > sim_num:
        sim_tasks[task_id] = np.random.choice(sim_tasks[task_id], sim_num, replace=False)
...
def train(args, model, device, x,y, optimizer, criterion, task_id_nominal, consolidated_masks, feature_matrix, per_task_masks, task_relation, epoch):
    ...
    for i in range(0,len(r),args.batch_size_train):
        ...
        for i in sims:
            ...
            output = model(data, i, mask=mask, mode="train", epoch=epoch, end_epoch=args.n_epochs//50)
...
def main(args):
    ...
    data,taskcla,inputsize=cifar100.get(seed=args.seed, pc_valid=args.pc_valid)
    ...
    for k, ncla in taskcla:
        ...
        mat_list = get_representation_matrix(model, device, data[task_id]['train']['x'], task_id, model.get_masks(task_id), size=(-1, 3, 32, 32))
...
if __name__ == "__main__":
    ...
    #ETIL parameters 
    parser.add_argument("--sim_threshold", default=0.7, type=float, help="Threshold for ETIL to determine similar tasks.")
