...
def update_task_discrimination(model, task_id, feature_list_ori, feature_list_new, sim_tasks, dsim_tasks, threshold=0.7, sim_num=2):

    #计算训练后的下一个任务和原任务的距离
    distance_ori = []
    for t in range(task_id):
        if model.last[task_id].weight.shape[0] != model.last[t].weight.shape[0] or model.last[task_id].weight.shape[1] != model.last[t].weight.shape[1]:
            distance_ori.append(0)
            continue
        distance_ori.append(
...
def train(args, model, device, x,y, optimizer, criterion, task_id_nominal, consolidated_masks, feature_matrix, per_task_masks, task_relation):
    ...
    for i in range(0,len(r),args.batch_size_train):
        ...
        for i in sims:
            if model.last[task_id_nominal].weight.shape[0] != model.last[i].weight.shape[0] or model.last[task_id_nominal].weight.shape[1] != model.last[i].weight.shape[1]:
                continue
...
def main(args):
    ...
    from dataloader import mixceleba
    data,taskcla,inputsize=mixceleba.get(seed=args.seed, pc_valid=args.pc_valid, idrandom=args.idrandom, n_tasks=args.n_tasks)
    ...
    # Model Instantiation
    model = MLP(taskcla, args.sparsity, args.n_hidden, input_size=(3*32*32)) # base filters: 20
    ...
    for k, ncla in taskcla:
        ...
        if len(args.sample_size > data[k]['train']['x']):
            data_buffer.append(data[k]['train']['x'][:15])
        else:
        ...
        if task_id > 0:
            ...
            update_task_discrimination(model, task_id, feature_list_ori, feature_list_new[task_id], sim_tasks, dsim_tasks, threshold=args.sim_threshold)
            print(sim_tasks)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--k_threshold", default=0.955, type=float, help="Threshold for choose Orthogonal projection bases of SVD.")
