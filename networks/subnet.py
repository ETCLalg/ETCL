...
class GetDesignateSubnet(torch.autograd.Function):

    @staticmethod
    def forward(ctx, scores, masks, zeros, ones):
        #mask为1的部分scores为1，mask为0的部分scores为0
        scores*=masks
        return torch.where(scores == 0, zeros.to(scores.device), ones.to(scores.device))
...
class SubnetLinear(nn.Linear):
    ...
    def get_gpm(self, x, weights):
        with torch.no_grad():
            ...
            feat=U[:,0:r]
            self.Uf=torch.Tensor(np.dot(feat,feat.T)).to(weights.device)


    def get_representation_matrix(self, x):
        with torch.no_grad():
            ...
            # torch.mm(x[b_idx,], w_pruned.t()).t().cpu().numpy()
            ...
            # feat=U[:,0:r]
            # return torch.Tensor(np.dot(feat,feat.transpose())).to(self.weight.device)
    ...
    def forward(self, x, weight_mask=None, bias_mask=None, mode="train", epoch=-1, end_epoch=0):
        ...
        if mode == "train":
            if weight_mask is None:
                ...
                if epoch < end_epoch:
                    self.weight_mask = GetDesignateSubnet.apply(self.w_m.abs(),
                                                                weight_mask,
                                                                self.zeros_weight,
                                                                self.ones_weight,
                                                                )
