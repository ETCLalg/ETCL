...
def get_representation_matrix(net,device,x,task_id=0,mask=None,mode='valid',random=False, size=(-1, 28*28), batch_list=None):
    with torch.no_grad():
        ...
        example_data = x[b].view(size)
        example_data = example_data.to(device)
        example_out  = net(example_data, task_id, mask, mode, size=size[1:])
...
def get_representation_matrix_and_gradient(net,device,x,task_id=0,mask=None,mode='valid',random=True, size=(-1, 28*28), regime3=True, y=None, optimizer=None, criterion=None):
    net = net.to(device)
    ...
    mat_list=[] # list contains representation matrix of each layer
    grad_list=[] # list contains gradient matrix of each layer
    act_key=list(net.act.keys())
    ...
    if regime3:
        ...
        for k, (m, param) in enumerate(net.named_parameters()):
            if 'w_m' in m:
                grad_list.append(param.grad.data.detach().cpu().numpy())
