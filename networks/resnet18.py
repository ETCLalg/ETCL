...
class mySequential(nn.Sequential):
    def forward(self, *inputs, epoch=-1, end_epoch=0):
        mask = inputs[1]
        mode = inputs[2]
        inputs = inputs[0]
        for module in self._modules.values():
            if isinstance(module, SubnetBasicBlock):
                inputs = module(inputs, mask, mode, epoch=epoch, end_epoch=end_epoch)
            else:
...
class SubnetBasicBlock(nn.Module):
    ...
    def forward(self, x, mask, mode='train', epoch=-1, end_epoch=0):
        self.count = self.count % 2
        self.act['conv_{}'.format(self.count)] = x
        self.count += 1

        name = self.name + ".conv1"
        out = relu(self.bn1(self.conv1(x, weight_mask=mask[name+'.weight'], bias_mask=mask[name+'.bias'], mode=mode, epoch=epoch, end_epoch=end_epoch)))

        self.count = self.count % 2
        self.act['conv_{}'.format(self.count)] = out
        self.count += 1
        name = self.name + ".conv2"
        out = self.bn2(self.conv2(out, weight_mask=mask[name+'.weight'], bias_mask=mask[name+'.bias'], mode=mode, epoch=epoch, end_epoch=end_epoch))
        if self.shortcut is not None:
            name = self.name + ".conv3"
            out += self.bn3(self.conv3(x, weight_mask=mask[name+'.weight'], bias_mask=mask[name+'.bias'], mode=mode, epoch=epoch, end_epoch=end_epoch))
        else:
            out += x
        out = relu(out)
        return out
...
class SubnetResNet(nn.Module):
    ...
    def forward(self, x, task_id, mask, mode="train", size=(3, 32, 32), epoch=-1, end_epoch=0):
        if mask is None:
            mask = self.none_masks

        bsz = x.size(0)
        x = x.reshape(bsz, *size)
        out = relu(self.bn1(self.conv1(x, weight_mask=mask['conv1.weight'], bias_mask=mask['conv1.bias'], mode=mode)))
        out = self.layer1(out, mask, mode, epoch=epoch, end_epoch=end_epoch)
        out = self.layer2(out, mask, mode, epoch=epoch, end_epoch=end_epoch)
        out = self.layer3(out, mask, mode, epoch=epoch, end_epoch=end_epoch)
        out = self.layer4(out, mask, mode, epoch=epoch, end_epoch=end_epoch)
        out = adaptive_avg_pool2d(out, 2)
        out = out.reshape(out.size(0), -1)
        self.act['last.' + str(task_id)] = out
...
class STLResNet(nn.Module):
    ...
    def forward(self, x, size):
        bsz = x.size(0)
        out = relu(self.bn1(self.conv1(x.view(bsz, *size)))) 
        out = self.layer1(out)
        out = self.layer2(out)
        out = self.layer3(out)
        out = self.layer4(out)
