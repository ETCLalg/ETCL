...
class SubnetAlexNet_norm(nn.Module):
    ...
    def forward(self, x, task_id, mask, mode="train", size=-1, epoch=-1, end_epoch=0):
        ...
        bsz = deepcopy(x.size(0))
        x = self.conv1(x, weight_mask=mask['conv1.weight'], bias_mask=mask['conv1.bias'], mode=mode, epoch=epoch, end_epoch=end_epoch)
        x = self.maxpool(self.drop1(self.relu(self.bn1(x))))

        x = self.conv2(x, weight_mask=mask['conv2.weight'], bias_mask=mask['conv2.bias'], mode=mode, epoch=epoch, end_epoch=end_epoch)
        x = self.maxpool(self.drop1(self.relu(self.bn2(x))))

        x = self.conv3(x, weight_mask=mask['conv3.weight'], bias_mask=mask['conv3.bias'], mode=mode, epoch=epoch, end_epoch=end_epoch)
        x = self.maxpool(self.drop2(self.relu(self.bn3(x))))
